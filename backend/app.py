...
def startup_event():
    init_chat_session()
    logger.info("Gemini Chatbot backend started successfully and is running healthy.")


@app.get("/")
def root():
    return {
        "message": "Gemini Chatbot backend is running healthy.",
        "status": "ok",
        "health": "/api/health",
        "chat": "/api/chat",
    }
...
@app.get("/api/health")
def health_check():
    logger.info("Health check requested: backend is healthy.")
    return {"status": "ok", "message": "Gemini Chatbot backend is running healthy."}
